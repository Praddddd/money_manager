...
// ═══════════════════════════════════════════════════════════════════════════════
// ADD EXPENSE PAGE V2 — REDESIGNED
// ═══════════════════════════════════════════════════════════════════════════════
...
class _AddExpensePageV2State extends State<AddExpensePageV2> {
  ...
  void _showScanPreview(OcrResult result) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text('Verifikasi Data Scan', style: GoogleFonts.inter(fontSize: 18, fontWeight: FontWeight.w700, color: C.t1)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
        ...
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: Text('Batal', style: GoogleFonts.inter(color: C.t2))),
          TextButton(onPressed: () { Navigator.pop(ctx); _applyScanResult(formattedAmount, noteToShow, categoryToShow); }, child: Text('Terapkan', style: GoogleFonts.inter(color: C.accent, fontWeight: FontWeight.w700))),
        ],
      ),
    );
  }
  ...
  Widget _previewField(String label, String value, Color color) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(label, style: GoogleFonts.inter(fontSize: 11, color: C.t3, fontWeight: FontWeight.w600)),
        const SizedBox(height: 6),
        Container(
          padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
          decoration: BoxDecoration(color: C.elevated, borderRadius: BorderRadius.circular(12), border: Border.all(color: color.withValues(alpha: 0.2), width: 1.5)),
          child: Text(value, style: GoogleFonts.inter(fontSize: 14, color: color, fontWeight: FontWeight.w600)),
        ),
      ],
    );
  }
  ...
  void _showError(String msg) {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Row(children: [const Icon(Icons.error_outline_rounded, color: C.red, size: 20), const SizedBox(width: 10), Expanded(child: Text(msg, style: const TextStyle(color: Colors.white)))]), backgroundColor: C.card, behavior: SnackBarBehavior.floating, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))));
  }

  void _showSuccess(String msg) {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Row(children: [const Icon(Icons.check_circle_rounded, color: C.green, size: 20), const SizedBox(width: 10), Expanded(child: Text(msg, style: const TextStyle(color: Colors.white)))]), backgroundColor: C.card, behavior: SnackBarBehavior.floating, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), duration: const Duration(seconds: 2)));
  }
  ...
  Future<void> _submit() async {
    ...
    final expense = Expense(id: DateTime.now().millisecondsSinceEpoch.toString(), amount: double.parse(_amtCtrl.text.replaceAll('.', '')), note: _noteCtrl.text.trim(), category: _cat!, date: DateTime.now());
    await context.read<ExpenseProvider>().add(expense);
    _amtCtrl.clear();
    _noteCtrl.clear();
    setState(() { _cat = null; _submitting = false; });
    if (!mounted) return;
      ...
      child: SingleChildScrollView(
        ...
        child: Form(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Tambah Pengeluaran', style: GoogleFonts.inter(fontSize: 28, fontWeight: FontWeight.w900, color: C.t1, letterSpacing: -0.8)),
              const SizedBox(height: 8),
              Text('Catat pengeluaran baru kamu', style: GoogleFonts.inter(fontSize: 14, color: C.t3)),
              const SizedBox(height: 32),
              Text('JUMLAH', style: GoogleFonts.inter(fontSize: 12, color: C.t3, fontWeight: FontWeight.w700, letterSpacing: 1)),
              const SizedBox(height: 10),
              Container(
                decoration: BoxDecoration(borderRadius: BorderRadius.circular(16), border: Border.all(color: C.divider, width: 1)),
                child: TextFormField(
                  controller: _amtCtrl,
                  keyboardType: TextInputType.number,
                  style: GoogleFonts.inter(fontSize: 24, fontWeight: FontWeight.w800, color: C.t1),
                  decoration: InputDecoration(hintText: '0', prefixText: 'Rp ', prefixStyle: GoogleFonts.inter(fontSize: 18, fontWeight: FontWeight.w700, color: C.t2), border: InputBorder.none, contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14), filled: true, fillColor: C.elevated.withValues(alpha: 0.5)),
                  validator: (v) { if (v == null || v.isEmpty) return 'Masukkan jumlah'; final clean = v.replaceAll('.', ''); if (double.tryParse(clean) == null) return 'Jumlah tidak valid'; return null; },
                ),
              ),
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(borderRadius: BorderRadius.circular(12), border: Border.all(color: C.accent, width: 2), color: C.accent.withValues(alpha: 0.1)),
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Icon(Icons.document_scanner_rounded, color: C.accent, size: 18),
                      const SizedBox(width: 8),
                      Text(_scanning ? 'Memproses...' : 'Scan Struk', style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w700, color: C.accent)),
                      if (_scanning) ...[const SizedBox(width: 8), const SizedBox(width: 12, height: 12, child: CircularProgressIndicator(color: C.accent, strokeWidth: 2))],
                    ],
                  ),
                ),
              ),
              const SizedBox(height: 28),
              Text('KATEGORI', style: GoogleFonts.inter(fontSize: 12, color: C.t3, fontWeight: FontWeight.w700, letterSpacing: 1)),
              const SizedBox(height: 12),
              Wrap(
                ...
                children: Cat.all.map((cat) {
                  ...
                  return GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(color: active ? C.accent : C.elevated, borderRadius: BorderRadius.circular(12), border: Border.all(color: active ? C.accent : C.divider, width: 1.5)),
                      child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(Cat.icon(cat), size: 16, color: active ? Colors.white : C.t2), const SizedBox(width: 6), Text(cat, style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w600, color: active ? Colors.white : C.t2))]),
                    ),
                  );
                }).toList(),
              ),
              const SizedBox(height: 28),
              Text('CATATAN', style: GoogleFonts.inter(fontSize: 12, color: C.t3, fontWeight: FontWeight.w700, letterSpacing: 1)),
              const SizedBox(height: 10),
              Container(
                decoration: BoxDecoration(borderRadius: BorderRadius.circular(16), border: Border.all(color: C.divider, width: 1)),
                child: TextFormField(
                  ...
                  decoration: InputDecoration(hintText: 'Deskripsi pengeluaran (opsional)', border: InputBorder.none, contentPadding: const EdgeInsets.all(14), filled: true, fillColor: C.elevated.withValues(alpha: 0.5), hintStyle: GoogleFonts.inter(fontSize: 14, color: C.t4)),
                ),
              ),
              const SizedBox(height: 32),
              GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(color: _submitting ? C.accent.withValues(alpha: 0.5) : C.accent, borderRadius: BorderRadius.circular(14), boxShadow: [BoxShadow(color: C.accent.withValues(alpha: 0.3), blurRadius: 12, offset: const Offset(0, 4))]),
                  child: Center(
                    child: _submitting ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2.5)) : Text('Simpan Pengeluaran', style: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.w700, color: Colors.white)),
                  ),
                ),
              ),
              const SizedBox(height: 20),
